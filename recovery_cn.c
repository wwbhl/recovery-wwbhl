...
wipe_data(int confirm) {
    ...
    if (confirm && !confirm_with_headers(headers, "Yes - 清除所有数据")) {
        return;
    }
...
ui_print("wwbhl 汉化编译("EXPAND(RECOVERY_BUILD_DATE)")\n");
#ifdef PHILZ_TOUCH_RECOVERY
    ...
    if (update_package != NULL) {
        ...
        if (wipe_cache && erase_volume("/cache")) status = INSTALL_ERROR;
        if (status != INSTALL_SUCCESS)  ui_print("清除Cache失败.\n");
    } else if (wipe_media) {
        if (is_data_media() && erase_volume("/data/media")) status = INSTALL_ERROR;
        if (status != INSTALL_SUCCESS) ui_print("清除Media失败..\n");
    } else if (sideload) {
