...
void wipe_data_menu() {
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 1: {
                if (confirm_selection("清除cache分区?", "Yes - 清除cache")) {
                    ui_print("\n-- 正在清除 cache...\n");
            ...
            case 2: {
                ...
                if (confirm_selection("清除dalvik cache ?", "Yes - 清除dalvik cache")) {
                    ui_print("\n-- 正在清除dalvik cache...\n");
                    __system("rm -r /data/dalvik-cache");
                    __system("rm -r /cache/dalvik-cache");
                    __system("rm -r /sd-ext/dalvik-cache");
                    ui_print("Dalvik Cache清除完毕.\n");
                }
                break;
            }
            case 3: {
                ...
                if (confirm_with_headers(headers, "Yes - 清除")) {
                    wipe_data(false);
                    ui_print("-- 正在清除system...\n");
                    erase_volume("/system");
                    if (volume_for_path("/preload") != NULL) {
                        ui_print("-- 正在清除preload...\n");
            ...
            case 4: {
                ...
                if (confirm_with_headers(headers, "Yes - 格式化")) {
                    ui_print("\n-- 正在格式化...\n");
...
void show_multi_flash_menu() {
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        if (no_files_found) {
            ui_print("在%s目录下建立至少一个zip文件\n", tmp);
            ui_print("从其他存储查找...\n");
        }
    } else {
        LOGI("%s 未找到.正在从其他存储搜索...\n", tmp);
    }
    ...
    if (numFiles == 0) {
        ...
        if (chosen_item == 1) {
            ...
            sprintf(confirm, "Yes - 从%s安装", BaseName(zip_folder));
            if (confirm_selection("安装所选文件?", confirm)) {
                for(i = 2; i < numFiles + 2; i++) {
...
static void choose_ors_volume() {
    ...
    const char* headers[] = { "保存ors备份到:", NULL };
...
static void choose_custom_ors_menu(const char* volume_path) {
    ...
    const char* headers[] = {"选择.ors脚本运行", NULL};
    ...
    char confirm[PATH_MAX];
    sprintf(confirm, "Yes - 运行 %s", BaseName(ors_file));
    if (confirm_selection("确定运行?", confirm)) {
        run_ors_script(ors_file);
    }
...
static void show_custom_ors_menu() {
    ...
    const char* headers[] = { "搜索.ors脚本运行", "", NULL };
...
static void regenerate_md5_sum_menu() {
    if (!confirm_selection("不推荐!!", "Yes - 重新生成"))
        return;
...
void misc_nandroid_menu() {
    ...
    int fmt;
    for (;;) {
        if (enable_md5sum.value) ui_format_gui_menu(item_md5, "MD5校验", "(Y)");
        else ui_format_gui_menu(item_md5, "MD5", "(N)");

        if (volume_for_path("/preload") == NULL)
            ui_format_gui_menu(item_preload, "包含/preload", "N/A");
        else if (nandroid_add_preload.value) ui_format_gui_menu(item_preload, "包含/preload", "(Y)");
        else ui_format_gui_menu(item_preload, "包含/preload", "(N)");

        if (twrp_backup_mode.value) ui_format_gui_menu(item_twrp_mode, "Use TWRP Mode", "(Y)");
        else ui_format_gui_menu(item_twrp_mode, "使用TWRP模式", "(N)");

        if (show_nandroid_size_progress.value)
            ui_format_gui_menu(item_size_progress, "显示nandroid进程大小", "(Y)");
        else ui_format_gui_menu(item_size_progress, "显示nandroid进程大小", "(N)");
        list[3] = item_size_progress;

        if (use_nandroid_simple_logging.value)
            ui_format_gui_menu(item_use_nandroid_simple_logging, "使用简单的日志(更快)", "(Y)");
        else ui_format_gui_menu(item_use_nandroid_simple_logging, "使用简单的日志(更快)", "(N)");
        list[4] = item_use_nandroid_simple_logging;

        hidenandprogress = file_found(hidenandprogress_file);
        if (hidenandprogress) {
            ui_format_gui_menu(item_nand_progress, "隐藏Nandroid进程", "(Y)");
            list[3] = NULL;
            list[4] = NULL;
        } else ui_format_gui_menu(item_nand_progress, "隐藏Nandroid进程", "(N)");

        if (nand_prompt_on_low_space.value)
            ui_format_gui_menu(item_prompt_low_space, "空间不足提示", "(Y)");
        else ui_format_gui_menu(item_prompt_low_space, "空间不足提示", "(N)");
...
void set_custom_zip_path() {
    ...
    const char* headers[] = { "设置自由目录", NULL };
...
void show_twrp_restore_menu(const char* backup_volume) {
    ...
    const char* headers[] = { "选择一个备份还原", NULL };
    ...
    sprintf(backup_source, "%s", DirName(file));
    ui_print("%s 将还原到所选分区!\n", backup_source);
    sprintf(confirm, "Yes - 还原 %s", BaseName(backup_source));
    if (confirm_selection("从当前备份还原 ?", confirm))
...
static void custom_restore_handler(const char* backup_volume, const char* backup_folder) {
    ...
    if (backup_efs == RESTORE_EFS_IMG) {
        ...
        ui_print("%s将刷写到/efs!\n", backup_source);
        sprintf(tmp, "Yes - 还原 %s", backup_source);
        ...
        if (file_found(tmp)) {
            ui_print("从%s删除efs.img!\n", file);
            ui_print("选择efs.img来还原,\n");
            ui_print("或者删除以还原nandroid source.\n");
        } else {
            // restore efs from nandroid tar format
            ui_print("%s将还原到/efs!\n", file);
            sprintf(tmp, "Yes - 还原 %s", BaseName(file));
            if (confirm_selection(confirm_install, tmp))
        ...
        sprintf(backup_source, "%s", DirName(file));
        ui_print("%s 还原到选择的分区!\n", backup_source);
        sprintf(tmp, "Yes - 还原%s", BaseName(backup_source));
        if (confirm_selection(confirm_install, tmp)) {
...
void custom_restore_menu(const char* backup_volume) {
    ...
    for (;;)
    {
        ...
        list[LIST_ITEM_VALIDATE] = strdup(">> 启动自定义还原");

        if (reboot_after_nandroid) ui_format_gui_menu(menu_item_tmp, ">> 完成后重启", "(Y)");
        else ui_format_gui_menu(menu_item_tmp, ">> 完成后重启", "(N)");
        list[LIST_ITEM_REBOOT] = strdup(menu_item_tmp);

        if (volume_for_path(BOOT_PARTITION_MOUNT_POINT) != NULL) {
            if (backup_boot) ui_format_gui_menu(menu_item_tmp, "还原 boot", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "还原 boot", "(N)");
        ...
        if (volume_for_path("/recovery") != NULL) {
            if (backup_recovery) ui_format_gui_menu(menu_item_tmp, "还原 recovery", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "还原 recovery", "(N)");
            list[LIST_ITEM_RECOVERY] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_RECOVERY] = NULL;
        }

        if (backup_system) ui_format_gui_menu(menu_item_tmp, "还原 system", "(Y)");
        else ui_format_gui_menu(menu_item_tmp, "还原 system", "(N)");
        list[LIST_ITEM_SYSTEM] = strdup(menu_item_tmp);

        if (volume_for_path("/preload") != NULL) {
            if (backup_preload) ui_format_gui_menu(menu_item_tmp, "还原 preload", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "还原 preload", "(N)");
            list[LIST_ITEM_PRELOAD] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_PRELOAD] = NULL;
        }

        if (backup_data) ui_format_gui_menu(menu_item_tmp, "还原 data", "(Y)");
        else ui_format_gui_menu(menu_item_tmp, "还原 data", "(N)");
        list[LIST_ITEM_DATA] = strdup(menu_item_tmp);

        set_android_secure_path(tmp);
        if (backup_data && android_secure_ext)
            ui_format_gui_menu(menu_item_tmp, "还原 and-sec", DirName(tmp));
        else ui_format_gui_menu(menu_item_tmp, "还原 and-sec", "(N)");
        list[LIST_ITEM_ANDSEC] = strdup(menu_item_tmp);

        if (backup_cache) ui_format_gui_menu(menu_item_tmp, "还原 cache", "(Y)");
        else ui_format_gui_menu(menu_item_tmp, "还原 cache", "(N)");
        list[LIST_ITEM_CACHE] = strdup(menu_item_tmp);

        if (backup_sdext) ui_format_gui_menu(menu_item_tmp, "还原 sd-ext", "(Y)");
        else ui_format_gui_menu(menu_item_tmp, "还原 sd-ext", "(N)");
        list[LIST_ITEM_SDEXT] = strdup(menu_item_tmp);

        if (volume_for_path("/modem") != NULL) {
            if (backup_modem == RAW_IMG_FILE)
                ui_format_gui_menu(menu_item_tmp, "还原 modem [.img]", "(Y)");
            else if (backup_modem == RAW_BIN_FILE)
                ui_format_gui_menu(menu_item_tmp, "还原 modem [.bin]", "(N)");
            else ui_format_gui_menu(menu_item_tmp, "还原 modem", "( )");
        ...
        if (volume_for_path("/radio") != NULL) {
            if (backup_radio == RAW_IMG_FILE)
                ui_format_gui_menu(menu_item_tmp, "还原 radio [.img]", "(Y)");
            else if (backup_radio == RAW_BIN_FILE)
                ui_format_gui_menu(menu_item_tmp, "还原 radio [.bin]", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "还原 radio", "(N)");
        ...
        if (volume_for_path("/efs") != NULL) {
            if (backup_efs == RESTORE_EFS_IMG)
                ui_format_gui_menu(menu_item_tmp, "还原 efs [.img]", "(Y)");
            else if (backup_efs == RESTORE_EFS_TAR)
                ui_format_gui_menu(menu_item_tmp, "还原 efs [.tar]", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "还原 efs", "(N)");
        ...
        if (volume_for_path("/misc") != NULL) {
            if (backup_misc) ui_format_gui_menu(menu_item_tmp, "还原 misc", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "还原 misc", "(N)");
        ...
        if (is_data_media() && !twrp_backup_mode.value) {
            if (backup_data_media)
                ui_format_gui_menu(menu_item_tmp, "还原 /data/media", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "还原 /data/media", "(N)");
        ...
        if (volume_for_path("/wimax") != NULL && !twrp_backup_mode.value) {
            if (backup_wimax)
                ui_format_gui_menu(menu_item_tmp, "还原 WiMax", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "还原 WiMax", "(N)");
...
void custom_backup_menu(const char* backup_volume)
{
    ...
    for (;;)
    {
        ...
        list[LIST_ITEM_VALIDATE] = strdup(">> 启动自定义备份");
        ...
        if (volume_for_path(BOOT_PARTITION_MOUNT_POINT) != NULL) {
            if (backup_boot) ui_format_gui_menu(menu_item_tmp, "备份 boot", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "备份 boot", "(N)");
            list[LIST_ITEM_BOOT] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_BOOT] = NULL;
        }
        ...
        if (volume_for_path("/recovery") != NULL) {
            if (backup_recovery) ui_format_gui_menu(menu_item_tmp, "备份 recovery", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "备份 recovery", "(N)");
            list[LIST_ITEM_RECOVERY] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_RECOVERY] = NULL;
        }

        if (backup_system) ui_format_gui_menu(menu_item_tmp, "备份 system", "(Y)");
        else ui_format_gui_menu(menu_item_tmp, "备份 system", "(N)");
        list[LIST_ITEM_SYSTEM] = strdup(menu_item_tmp);

        if (volume_for_path("/preload") != NULL) {
            if (backup_preload) ui_format_gui_menu(menu_item_tmp, "备份 preload", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "备份 preload", "(N)");
            list[LIST_ITEM_PRELOAD] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_PRELOAD] = NULL;
        }

        if (backup_data) ui_format_gui_menu(menu_item_tmp, "备份 data", "(Y)");
        else ui_format_gui_menu(menu_item_tmp, "备份 data", "(N)");
        list[LIST_ITEM_DATA] = strdup(menu_item_tmp);

        set_android_secure_path(tmp);
        if (backup_data && android_secure_ext)
            ui_format_gui_menu(menu_item_tmp, "备份 and-sec", DirName(tmp));
        else ui_format_gui_menu(menu_item_tmp, "备份 and-sec", "(N)");
        list[LIST_ITEM_ANDSEC] = strdup(menu_item_tmp);

        if (backup_cache) ui_format_gui_menu(menu_item_tmp, "备份 cache", "(Y)");
        else ui_format_gui_menu(menu_item_tmp, "备份 cache", "(N)");
        list[LIST_ITEM_CACHE] = strdup(menu_item_tmp);

        if (backup_sdext) ui_format_gui_menu(menu_item_tmp, "备份 sd-ext", "(Y)");
        else ui_format_gui_menu(menu_item_tmp, "备份 sd-ext", "(N)");
        list[LIST_ITEM_SDEXT] = strdup(menu_item_tmp);

        if (volume_for_path("/modem") != NULL) {
            if (backup_modem == RAW_IMG_FILE)
                ui_format_gui_menu(menu_item_tmp, "备份 modem [.img]", "(Y)");
            else if (backup_modem == RAW_BIN_FILE)
                ui_format_gui_menu(menu_item_tmp, "备份 modem [.bin]", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "备份 modem", "(N)");
            list[LIST_ITEM_MODEM] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_MODEM] = NULL;
        }
        ...
        if (volume_for_path("/radio") != NULL) {
            if (backup_radio == RAW_IMG_FILE)
                ui_format_gui_menu(menu_item_tmp, "备份 radio [.img]", "(Y)");
            else if (backup_radio == RAW_BIN_FILE)
                ui_format_gui_menu(menu_item_tmp, "备份 radio [.bin]", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "备份 radio", "(N)");
            list[LIST_ITEM_RADIO] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_RADIO] = NULL;
        }
        ...
        if (volume_for_path("/efs") != NULL) {
            if (backup_efs == RESTORE_EFS_IMG)
                ui_format_gui_menu(menu_item_tmp, "备份 efs [.img]", "(Y)");
            else if (backup_efs == RESTORE_EFS_TAR)
                ui_format_gui_menu(menu_item_tmp, "备份 efs [.tar]", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "备份 efs", "(N)");
            list[LIST_ITEM_EFS] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_EFS] = NULL;
        }
        ...
        if (volume_for_path("/misc") != NULL) {
            if (backup_misc) ui_format_gui_menu(menu_item_tmp, "备份 misc", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "备份 misc", "(N)");
            list[LIST_ITEM_MISC] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_MISC] = NULL;
        }
        ...
        if (is_data_media() && !twrp_backup_mode.value) {
            if (backup_data_media)
                ui_format_gui_menu(menu_item_tmp, "备份 /data/media", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "备份 /data/media", "(N)");
            list[LIST_ITEM_DATAMEDIA] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_DATAMEDIA] = NULL;
        }
        ...
        if (volume_for_path("/wimax") != NULL && !twrp_backup_mode.value) {
            if (backup_wimax)
                ui_format_gui_menu(menu_item_tmp, "备份 WiMax", "(Y)");
            else ui_format_gui_menu(menu_item_tmp, "备份 WiMax", "(N)");
            list[LIST_ITEM_WIMAX] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_WIMAX] = NULL;
        }
...
static void import_export_settings() {
    const char* headers[] = { "备份/还原设置", "", NULL };
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 4: {
                ...
                if (theme_file == NULL)
                    break;
                if (confirm_selection("删除以选择主题?", "Yes - 删除"))
...
void show_philz_settings_menu()
{
    ...
    for (;;) {
        if (check_root_and_recovery.value)
            ui_format_gui_menu(item_check_root_and_recovery, "退出时验证Root", "(Y)");
        else ui_format_gui_menu(item_check_root_and_recovery, "退出时验证是Root", "(N)");

        if (auto_restore_settings.value)
            ui_format_gui_menu(item_auto_restore, "自动还原设置", "(Y)");
        else ui_format_gui_menu(item_auto_restore, "自动还原设置", "(N)");
        int chosen_item = get_menu_selection(headers, list, 0, 0);
        if (chosen_item == GO_BACK)
            break;
        ...
        switch (chosen_item) {
            ...
            case 3: {
                if (confirm_selection("重置所有设置到默认?", "Yes,重置")) {
                    char settings_copy[PATH_MAX];
                    sprintf(settings_copy, "%s/%s", get_primary_storage_path(), PHILZ_SETTINGS_FILE2);
...
int show_install_update_menu() {
    ...
    const char* headers[] = { "安装zip刷机包", "", NULL };
    ...
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes]     = "自由浏览目录安装";
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 1] = "从上次安装目录读取刷机包";
    install_menu_items[FIXED_TOP_INSTALL_ZIP_MENUS + num_extra_volumes + 2] = "从adb sideload读取刷机包";
    ...
    for (;;) {
        if (signature_check_enabled.value)
            ui_format_gui_menu(item_toggle_signature_check, "刷机包签名校验", "(N)");
        else ui_format_gui_menu(item_toggle_signature_check, "刷机包签名校验", "(Y)");

        if (install_zip_verify_md5.value)
            ui_format_gui_menu(item_install_zip_verify_md5, "刷机包MD5值校验", "(N)");
        else ui_format_gui_menu(item_install_zip_verify_md5, "刷机包MD5值校验", "(Y)");
...
void show_choose_zip_menu(const char *mount_point) {
    ...
    const char* headers[] = { "选择一个刷机包", NULL };
...
void show_nandroid_restore_menu(const char* path) {
    ...
    if (confirm_selection("确认还原备份?", "Yes,还原"))
...
void show_nandroid_delete_menu(const char* volume_path) {
    ...
    const char* headers[] = { "选择一个备份删除", NULL };
    char path[PATH_MAX];
    char tmp[PATH_MAX];
    char* file;
    ...
    for(;;) {
        ...
        sprintf(tmp, "Yes,删除 %s", BaseName(file));
        if (confirm_selection("确认删除备份?", tmp)) {
...
static void show_format_ext4_or_f2fs_menu(const char* volume) {
    ...
    if (is_data_media() && strcmp(v->mount_point, "/data") == 0) {
        ...
        ret = confirm_with_headers(confirm_headers, "Yes,格式化设备");
    } else {
        ret = confirm_selection(confirm, "Yes,格式化设备");
    }
...
void show_partition_format_menu() {
    ...
    char* confirm_format = "确定要格式化?";
    char* confirm = "Yes - 格式化";
    ...
    for (;;) {
...
#ifdef USE_F2FS
            list[formatable_volumes] = "切换f2fs <-> ext4";
            list[formatable_volumes + 1] = NULL;
#endif
        } else {
            list[formatable_volumes] = "格式化 /data and /data/media (/sdcard)";
            list[formatable_volumes + 1] = NULL;
#ifdef USE_F2FS
            list[formatable_volumes + 1] = "切换f2fs <->";
        ...
        if (is_data_media() && chosen_item == formatable_volumes) {
...
#endif
            {
                if (!confirm_selection("格式化 /data and /data/media (/sdcard)", confirm))
                    continue;
                ui_print("正在格式化 /data...\n");
                preserve_data_media(0);
            ...
            ui_print("ext4 <-> f2fs 转换 %s\n", enable_f2fs_ext4_conversion ? "开启" : "关闭");
        }
#endif
    }
}
...
void choose_default_backup_format() {
    const char* headers[] = { "选择备份格式", "", NULL };
...
int show_nandroid_menu() {
    ...
    list[offset]     = "将ROM打包update.zip";
    list[offset + 1] = "清理无用备份数据";
    list[offset + 2] = "Nandroid设置";
...
void show_format_sdcard_menu(const char* path) {
    ...
    const char* headers[] = { "格式化:", path, "", NULL };
    ...
    if (chosen_item < 0) // REFRESH or GO_BACK
        return;
    if (!confirm_selection("确认格式化?", "Yes,格式化"))
        return;
    ...
    for (;;) {
        if (is_data_media()) {
            if (use_migrated_storage())
                list[6] = "Sdcard路径: /data/media/0";
            else list[6] = "Sdcard路径: /data/media";
        }
        ...
        if (enabled < 0) {
            ...
            if (enabled)
                ui_format_gui_menu(item_loki_toggle_menu, "Loki支持开关", "(Y)");
            else
                ui_format_gui_menu(item_loki_toggle_menu, "LoKi支持开关", "(N)");
        ...
        switch (chosen_item) {
            ...
            case 2: {
                if (confirm_selection("移出已存在的su?", "Yes - 安装 SuperSU")) {
...
int verify_root_and_recovery() {
    ...
    if (0 == lstat("/system/bin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            ...
            if (needs_suid && (st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_SetShowText(true);
                if (confirm_selection("Root权限丢失,是否修复?", "Yes - 修复 root (/system/bin/su)")) {
    ...
    if (0 == lstat("/system/xbin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            ...
            if (needs_suid && (st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_SetShowText(true);
                if (confirm_selection("Root权限丢失,是否修复?", "Yes - 修复 root (/system/xbin/su)")) {
    ...
    if (!exists || su_nums != 1) {
        ui_SetShowText(true);
        if (confirm_selection("root访问权限丢失/破坏. Root手机?", "Yes - 安装root (/system/xbin/su)")) {
