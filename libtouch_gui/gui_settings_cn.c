...
void show_touch_gui_menu() {
    ...
    for (;;) {
        if (touch_to_validate.value == NO_TOUCH_SUPPORT)
            ui_format_gui_menu(item_touch, "触控界面", "禁止");
        else if (touch_to_validate.value == TOUCH_HIGHLIGHT_ONLY)
            ui_format_gui_menu(item_touch, "触控界面", "只选");
        else if (touch_to_validate.value == DOUBLE_TAP_VALIDATION)
            ui_format_gui_menu(item_touch, "触控界面", "双击");
        else
        ...
        ui_format_gui_menu(item_logrows, "日志行设置", tmp);
        ...
        if (show_clock.value && show_battery.value)
            ui_format_gui_menu(item_batt_clock, "电量/时钟", "都显示");
        else if (show_clock.value)
            ui_format_gui_menu(item_batt_clock, "电量/时钟", "显示时钟");
        else if (show_battery.value)
            ui_format_gui_menu(item_batt_clock, "电量/时钟", "显示电量");
        else ui_format_gui_menu(item_batt_clock, "电量/时钟", "禁止显示");
...
void print_libtouch_version(int onscreen) {
    if (onscreen)
        ui_print("触控界面版本: " EXPAND(LIBTOUCH_GUI_VERSION) "\n");
    else
        LOGI("触控界面版本: " EXPAND(LIBTOUCH_GUI_VERSION) "\n");
}
